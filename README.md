...
FOCUST 面向食源性致病菌培养皿成像场景，构建以时序生长信号为核心的菌落检测、过滤与多分类识别系统，覆盖数据构建、训练、推理、评估与报告生成的完整工程链路。系统以四十帧序列作为标准样本单位，采用传统视觉候选与深度学习分类的混合架构，并通过双引擎配置支持 HCP 经典链路与 HCP 编码加 YOLO 链路。GUI 与 CLI 共享一致的配置与输出格式，满足离线与内网环境的交付要求。
...
在真实检测场景中，培养皿背景纹理、食品残渣、气泡、反光与划痕会显著抬高单帧检测的不确定性。系统以时间维度的变化与生长作为主要判别信号，在高干扰环境下形成可解释的候选生成与可训练的分类机制，并形成完整的训练与检测工作流。
...
本节给出仓库内保留的参考指标，用于说明系统能力的量级与稳定性。
